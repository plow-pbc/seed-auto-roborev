...
assert_contains "$ctx" "categorize EACH finding" "context mandates an out-loud per-finding triage, not an FYI"
assert_contains "$ctx" "INVALID (not a real problem)" "context teaches the INVALID verdict bucket"
assert_contains "$ctx" "VALID-BUT-YAGNI" "context teaches the three-way invalid/valid/valid-but-YAGNI verdict"
assert_contains "$ctx" "VALID (a real bug or a genuine simplification)" "context teaches the standalone VALID verdict bucket"
assert_contains "$ctx" "VERY NEXT" "context mandates fixing valid findings in the very next commit (strict, no batching to push)"
