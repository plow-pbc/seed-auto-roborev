...
- The seed does **not** layer its own confirmation/summary lines onto these hooks. The seed's purpose is to teach a *Claude Code agent* the review loop, and findings reach the agent through the three `PreToolUse[Bash]` hooks below (the [context bridge](#claude-specific-context-bridge) / [pre-push gate](#claude-specific-pre-push-gate) / [pre-checkout gate](#claude-specific-pre-checkout-gate)), not a terminal echo. A broken install (missing binary) surfaces to the agent via the bridge's loud context warning and on-demand via `verify.sh`.
...
- Before any Claude-Code `git checkout`/`git switch` to *another* branch (or a detach), the [pre-checkout gate](#claude-specific-pre-checkout-gate) **denies** the switch while the branch being **left** has open fail-verdict **or still-in-flight** reviews — the enforcement half of "drain before switching" (without it, a `verdict=F` strands on the abandoned branch, unseen by the per-current-branch push gate; an in-flight review could land `verdict=F` after the agent left). It tells the agent to `roborev wait` for in-flight ones and re-run the switch once drained. File restores (`git checkout -- <path>`, `git checkout .`, `git restore …`) are NOT gated.
