...
### Cross-branch backlog view

- Upstream `roborev list` has **no all-branches/all-repos mode** — it's scoped to one repo+branch — so a `verdict=F` review on a branch the agent has moved off of stays invisible to the per-branch surfaces. **This SEED installs** a `roborev-list-all.py` helper (to the same seed-owned `${XDG_CONFIG_HOME:-$HOME/.config}/roborev/claude-hooks/` path as the hooks) that reads the daemon's `~/.roborev/reviews.db` **directly and read-only** (a single `verdict_bool = 0 AND closed = 0` SELECT joining `reviews`/`review_jobs`/`repos`) and prints **every open fail-verdict review across all repos and branches** as `repo  branch  count/ids`, with **ephemeral fixture repos** (paths under `/tmp`, `/private/tmp`, `/var/folders`) filtered out as noise. It shares `_roborev_hooklib.py` with the hooks, so its DB-level open-finding predicate (`verdict_bool = 0`) is the same "outstanding finding" the hooks gate on at the CLI level (`verdict == "F" && !closed`) — the two definitions can't drift. The [pre-push gate](#claude-specific-pre-push-gate) **also surfaces this same backlog automatically** on its allow path (non-blocking `additionalContext`), so a clean push nudges the agent to sweep stale findings on other branches. The surface is **informational only**: the gate's hard deny stays strictly **current-branch** — an other-branch FAIL never blocks a push (that would wedge every push machine-wide). The cleanup policy is **active-vs-stale, not branch-ownership**: the agent resolves + `roborev close`es findings that are days-old / authored by it / invalid-or-YAGNI, but **never** closes a finding a parallel session is actively working (recently-created, branch checked out elsewhere, PR being iterated) — when unsure, it leaves it.
...
- MUST install the [cross-branch backlog view](#cross-branch-backlog-view) helper `roborev-list-all.py` alongside the hooks (same seed-owned `claude-hooks/` path, sharing `_roborev_hooklib.py`), so the agent can run the machine-wide open-FAIL sweep by hand and the gate can surface it.
...
### Open findings are surfaced before the next commit & before push
...
- **v-listall** — the [cross-branch backlog view](#cross-branch-backlog-view) helper `roborev-list-all.py` is installed executable alongside the hooks and, run against a `$HOME` with no `~/.roborev/reviews.db`, exits 1 (the distinct "couldn't read the DB" status) rather than crashing.
...
- **Branch-scoping still hides findings from the per-branch surfaces.** `roborev list`, the [context bridge](#claude-specific-context-bridge), and the [pre-push gate](#claude-specific-pre-push-gate)'s **hard deny** are all scoped to the **current repo + current branch** (a finding lands on whatever branch the commit was made on), so a finding on a branch you've moved off of doesn't trigger those per-branch surfaces. The [cross-branch backlog view](#cross-branch-backlog-view) closes the *visibility* half (the helper + the gate's non-blocking allow-path surface make the machine-wide backlog explicit), but the per-branch **gating** is deliberate: the deny stays current-branch-only so an other-branch FAIL never wedges a push machine-wide. The agent still resolves other-branch FAILs by sweeping the backlog (active-vs-stale), not by a hard block.
