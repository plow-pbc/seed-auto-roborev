...
**Drain before every push, before switching away from a branch, and again at task / PR handoff.** (The before-switch trigger is load-bearing: leaving a branch orphans its open findings — see the branch-scoping caveat below — so clear them while you're still on it.) Run `roborev list --open`, resolve every `verdict=F` (fix-then-close, or comment-then-close), and leave **zero** open fail-verdict reviews. **Never push over an open or unread `verdict=F`** — the pre-push gate is the backstop hard stop, but draining at commit time and at handoff keeps the eventual push unblocked, and a green push is not proof you read the findings.
...
**Drain only the branches YOU worked — the ownership unit is the branch.** With parallel agents on one repo, only `roborev close` reviews on a branch *you* committed to. Reading another branch's reviews — and even fixing a real bug they reveal — is fine, but **closing** a review on a branch you didn't touch marks it resolved on the owner's behalf: it buries a finding they never read and races their drain. Branch-scoped `roborev list --open` keeps you honest by default; the only way to violate this is `roborev close <id>` across branches, so don't "helpfully" sweep the machine — drain the branches you've committed on and leave the rest to whoever owns them.
