...
**This repo's `ref/` payload:** Python Claude Code `PreToolUse[Bash]` hooks (commit context bridge, pre-push + pre-checkout gates, shared hooklib), a cross-branch backlog helper (`roborev-list-all.py`), a seed-owned `post-commit` git-hook wrapper (skips pytest fixture repos, delegates to `roborev post-commit` otherwise), plus a bash install/test/verify harness. (roborev still supplies `post-rewrite` and the underlying `roborev post-commit` command via `roborev install-hook`; the installer overlays the wrapper on the `post-commit` file.)
