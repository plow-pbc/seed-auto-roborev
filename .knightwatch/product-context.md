...
**This repo's `ref/` payload:** Python Claude Code `PreToolUse[Bash]` hooks (commit context bridge, pre-push gate, pre-checkout gate, shared hooklib) plus a cross-branch backlog helper (`roborev-list-all.py`) and a bash install/test/verify harness. (roborev owns the actual git hooks — `post-commit`/`post-rewrite`, installed via `roborev install-hook` — not shipped in `ref/`.)
